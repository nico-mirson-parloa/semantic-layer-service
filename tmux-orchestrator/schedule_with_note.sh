...
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat \"$SCRIPT_DIR/next_check_note.txt\" && python3 \"$SCRIPT_DIR/tmux_utils.py\"' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
